...
class _MyProfileScreenState extends ConsumerState<MyProfileScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final posts = ref.watch(userPostsProvider(user.uid));

    final postNotifier = ref.read(userPostsProvider(user.uid).notifier);

    WidgetsBinding.instance.addPostFrameCallback((_) {
      postNotifier.fetchUserPosts(user.uid); // Fetch products
    });
    if (isLoading) {
      return const Center(
    ...
    return Row(
      ...
      children: [
        ...
        ElevatedButton.icon(
          ...
          icon: Icon(Icons.logout, color: ThemeUtils.sameBrightness(context)),
          onPressed: () async {
          ...
          label: Text('Sign Out',
              style: TextStyle(color: ThemeUtils.sameBrightness(context))),
        )
        // Unfollow Button
        ,
        SizedBox(width: 10),
  ...
  Row userStats(List<Post> posts, BuildContext context, UserModel user) {
    return Row(
